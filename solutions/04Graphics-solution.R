...
ggplot(data=dat,aes(x = TOTAL_COST_AMT,
                    group=SEX_CD)) + 
  geom_density(adjust=5,alpha=0.3) + 
  geom_vline(data=cost.summary, 
             aes(xintercept=med,  color=SEX_CD),
             linetype="dashed", size=1) + 
  geom_vline(data=cost.summary, 
             aes(xintercept=mn,  color=SEX_CD),
             linetype="solid", size=1) + 
  labs(x = "Total Cost Amount") +
  xlim(0,1000) # This will not show the long right tail so we can see the distributions better



# Overlay reference lines for the mean and median of only the 2003 facet Total Cost Amount distribution
cost.summary=dat %>% 
  group_by(SEX_CD, ADMIT_YR) %>% 
  summarise(mn = mean(TOTAL_COST_AMT), med=median(TOTAL_COST_AMT), cnt=length(TOTAL_COST_AMT))

cost.summary$med[cost.summary$ADMIT_YR!=2003] = NA
cost.summary$mn[cost.summary$ADMIT_YR!=2003] = NA
